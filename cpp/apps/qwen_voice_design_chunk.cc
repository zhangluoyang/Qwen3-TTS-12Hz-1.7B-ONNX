// chunk/pipeline C++ VoiceDesign command line entry.
//
// This is the streaming-flavored sibling of qwen_voice_design.cc: it calls
// GenerateVoiceDesignChunked() with an on_chunk callback so each decoded audio
...
int main(int argc, char** argv) {
  ...
  request.text = "你好，这是 Qwen 三 VoiceDesign 的 C++ chunk 流式测试。";
  ...
  std::filesystem::path output = "output_voice_design_cpp_chunk.wav";
  ...
  try {
    ...
    auto result = runtime.GenerateVoiceDesignChunked(request, chunk_options, on_chunk);
    ...
    if (print_timing) {
      ...
      std::cout << "\n[Timing] Overall\n" << std::fixed << std::setprecision(2)
                ...
                << "  total.generate_voice_design_chunked: " << generate_ms << " ms\n"
